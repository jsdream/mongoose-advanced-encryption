        ...
        async function processUpdate (update, path, options) {
            if (_.has(update, path)) {
                ...
                if (update[path]) {
                    ...
                    _.set(update, path + config.CIPHER_FIELD_SUFFIX, result.cipher);
                }

                if (options.hash) {
                    _.set(update, path + config.HASH_FIELD_SUFFIX, crypto.hash(_.get(update, path), pluginOptions.hashingKey));
                }
